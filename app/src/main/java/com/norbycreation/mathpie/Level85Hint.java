...
public class Level85Hint extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level85_hint);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            finish();
            Intent level85 = new Intent(getApplicationContext(), Level85.class);
            startActivity(level85);
            if (toggleStatus) {
    ...
    Spanned htmlTextWhiteMenuBack() {
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            return Html.fromHtml("<font color='#cfcfcf'>Level 85 Solution</font>", Html.FROM_HTML_MODE_LEGACY);
        } else {
            return Html.fromHtml("<font color='#cfcfcf'>Level 85 Solution</font>");
        }
    }
}