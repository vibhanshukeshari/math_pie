...
public class Level56Hint extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level56_hint);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            finish();
            Intent level56 = new Intent(getApplicationContext(), Level56.class);
            startActivity(level56);
            if (toggleStatus) {
    ...
    Spanned htmlTextWhiteMenuBack() {
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            return Html.fromHtml("<font color='#cfcfcf'>Level 56 Solution</font>", Html.FROM_HTML_MODE_LEGACY);
        } else {
            return Html.fromHtml("<font color='#cfcfcf'>Level 56 Solution</font>");
        }
    }
}