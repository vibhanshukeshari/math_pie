...
public class Level64Hint extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level64_hint);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            finish();
            Intent level64 = new Intent(getApplicationContext(), Level64.class);
            startActivity(level64);
            if (toggleStatus) {
    ...
    Spanned htmlTextWhiteMenuBack() {
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            return Html.fromHtml("<font color='#cfcfcf'>Level 64 Solution</font>", Html.FROM_HTML_MODE_LEGACY);
        } else {
            return Html.fromHtml("<font color='#cfcfcf'>Level 64 Solution</font>");
        }
    }
}