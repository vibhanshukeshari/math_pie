...
public class Level53Hint extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level53_hint);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            finish();
            Intent level53 = new Intent(getApplicationContext(), Level53.class);
            startActivity(level53);
            if (toggleStatus) {
    ...
    Spanned htmlTextWhiteMenuBack() {
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            return Html.fromHtml("<font color='#cfcfcf'>Level 53 Solution</font>", Html.FROM_HTML_MODE_LEGACY);
        } else {
            return Html.fromHtml("<font color='#cfcfcf'>Level 53 Solution</font>");
        }
    }
}