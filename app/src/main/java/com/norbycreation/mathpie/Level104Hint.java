...
public class Level104Hint extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level104_hint);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            finish();
            Intent level104 = new Intent(getApplicationContext(), Level104.class);
            startActivity(level104);
            if (toggleStatus) {
    ...
    Spanned htmlTextWhiteMenuBack() {
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            return Html.fromHtml("<font color='#cfcfcf'>Level 104 Solution</font>", Html.FROM_HTML_MODE_LEGACY);
        } else {
            return Html.fromHtml("<font color='#cfcfcf'>Level 104 Solution</font>");
        }
    }
}